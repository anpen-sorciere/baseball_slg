...
@section('title', 'nf3 打撃成績インポート')

@section('content')
    <h1>nf3 打撃成績 ペーストインポート</h1>
    <p>
    ...
    <form method="POST" action="{{ route('admin.nf3.batting_paste_import') }}">
        @csrf
        ...
        <div class="mb-3">
            <label for="raw_text" class="form-label">nf3 打撃成績テーブル（コピペ）</label>
            <textarea name="raw_text" id="raw_text" rows="20" class="form-control">{{ old('raw_text') }}</textarea>
            <small class="form-text text-muted">
                nf3の「打撃成績一覧」ページを開き、表部分をコピーしてここに貼り付けてください。
            </small>
        </div>
